...
@Slf4j
public class HealthPacketHandler implements CVPPacketHandler 
{
	...
	public CVPBasePacket processRequest(CVPData request) 
	{
		...
		if(request.getCvpPacketType().name() == CVPPacketType.HCHKR.name()) {
			log.info(" !!!!!!!!!!!!!!!!!!!!!!!!  I am in " + request.getCvpPacketType().name());
			log.info(" #### Health Data " + request.getRawdata());
			int cursorPos = 0;
			...
			HealthPacketVO healthPacketVO = HealthPacketVO.builder()
											...
											.build();
			basePkt = HealthPacket.builder().healthPacketVO(healthPacketVO).build();
