...
@Slf4j
public class EPBPacketHandler implements CVPPacketHandler 
{
	...
	public CVPBasePacket processRequest(CVPData request) 
	{
		...
		if(request.getCvpPacketType().name() == CVPPacketType.EPB.name()) {
			log.info(" !!!!!!!!!!!!!!!!!!!!!!!!  I am in " + request.getCvpPacketType().name());
			log.info(" #### EPB Data " + request.getRawdata());
			int cursorPos = 0;
			...
			String replyNumber = tokens[14];
			log.info(" #### replyNumber " + replyNumber);
			cursorPos = getNextCursorPosition(cursorPos + replyNumber.length());

			String checksum = tokens[15];
			log.info(" #### checksum " + checksum);
			cursorPos = getNextCursorPosition(cursorPos + checksum.length());
			
			String endCharacter = tokens[16];
			log.info(" #### endCharacter " + endCharacter);
			cursorPos = getNextCursorPosition(cursorPos + endCharacter.length());
			
			EBPPacketDataVO ebpPacketDataVO = EBPPacketDataVO.builder()
					...
					.build();
				basePkt = EBPPacket.builder().ebpPacketDataVO(ebpPacketDataVO).build();
		}else 
		{
