package com.bhavath.tracker.tcp.server;
...
public class TCPServer extends AbstractVerticle {
	...
	private CVPProcessor cvpProcessor;

	@Override
	public void start() throws Exception {
		
		log.info("TCPServer :: start :: start() ");
		...
		eb.consumer("next.topic", new Handler<Message<Buffer>>() {
			...
			public void handle(Message<Buffer> output) {
				try {
					log.info("TCPServer :: handle :: start() ");
					ByteArrayOutputStream output1 = new ByteArrayOutputStream();
					output1.write(output.body().getBytes());
					log.info("****Recieved total Data Packet:: "
							+ Hex.encodeHexString(output1.toByteArray()).toUpperCase());
					cvpProcessor.parse(Hex.encodeHexString(output1.toByteArray()).toUpperCase());
					log.info("TCPServer :: handle :: end() ");
				} catch (Exception e) {
					log.info("Exception in Vert.x:: " + e.getMessage());
				}
			}
